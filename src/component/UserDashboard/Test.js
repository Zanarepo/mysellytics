...
const AccessSwitcher = () => {
    ...
    try {
      ...
      if (uniqueStoreIds.length > 0) {
        uniqueStoreIds.forEach((storeId, index) => {
          opts.push({
            type: 'owner',
            label: `Single Store ${index + 1}`, // Add index to differentiate stores
            storeId,
            role: 'owner',
  ...
  const pickAccess = (opt, allAccess) => {
    const userAccess = {
      store_ids: opt.type === 'owner' ? [opt.storeId] : allAccess.filter((a) => a.storeId).map((a) => a.storeId), // Only selected store for owner, all for others
  ...
  const filteredAccessOptions = accessOptions.filter((opt) => {
    ...
    if (Array.isArray(currentRole)) {
      return !currentRole.includes(opt.type);
    }
