...
const Attendance = () => {
  ...
  const handleDeleteLog = async (logId) => {
    try {
      ...
      toast.success('Attendance log deleted.', { toastId: `delete-${logId}-success` });
    } catch (err) {
      console.error('handleDeleteLog error:', err);
      toast.error(err.message, { toastId: `delete-${logId}-error` });
    }
  };
      ...
      if (result) {
        try {
          ...
          if (currentHour < 6 || currentHour >= 21) {
            toast.error('Clocking only allowed between 6:00 AM and 9:00 PM.', { toastId: 'time-error' });
            return;
          }
          ...
          toast.success(
            `${user.full_name} ${action === 'clock-in' ? 'clocked in' : 'clocked out'} at ${format(
              now,
              'PPP HH:mm'
            )}.`,
      ...
      const scanCode = async () => {
        try {
          if (!webcamRef.current || !webcamRef.current.video) {
            ...
            toast.error('Webcam not available.', { toastId: 'webcam-error' });
            setScanning(false);
            return;
          }
          ...
          while (attempts < maxAttempts) {
            try {
              ...
              if (attempts === maxAttempts) {
                ...
                toast.error('Failed to access camera.', { toastId: 'camera-error' });
                setScanning(false);
                return;
              }
